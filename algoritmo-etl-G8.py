...
------------------------------------------------------------------------
MODIFICACIONES DEL EQUIPO 8 sobre el algoritmo original del docente:
  1) v_database se cambió a "bigdata" (la base de datos que se usa en el
     resto de la tarea).
  2) Al final del programa se agregó la función valorizarIdRegion(), que
     llena el campo "id_region" de la tabla "operaciones" a partir del
     campo "codigo_region" que ya trae cada departamento. Esto resuelve
     el punto 5 del informe (agregar y valorizar "id_region").
     Requisito previo: haber corrido "script-base-datos-regiones.sql"
     (crea la tabla "regiones" y la columna "operaciones.id_region").
------------------------------------------------------------------------
"""
#-------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------
# Función:  Valorizar el campo id_region en la tabla operaciones
# MODIFICACIÓN DEL EQUIPO 8 - Punto 5 del informe
# Cada operación hereda la región del departamento al que pertenece
# (departamentos.codigo_region), que ya se cargó más arriba.
#-----------------------------------------------------------------------------
def valorizarIdRegion(conn, cursor):
    try:
        comando_sql = '''UPDATE operaciones o
                          SET id_region = d.codigo_region
                          FROM departamentos d
                          WHERE d.id_departamento = o.id_departamento;'''
        cursor.execute(comando_sql)
        conn.commit()
        print("Campo id_region valorizado en 'operaciones'. Filas afectadas: ", cursor.rowcount)
    except (Exception, Error) as error:
        print("Error valorizando id_region: ", error)
    # Fin función valorizarIdRegion
...
try:
    ...
    with open(archivo_fisico) as File:
        ...
        # CORRECCIÓN DEL EQUIPO 8: el script original traía quotechar=','
        # (igual al delimiter), lo cual es inválido -> "bad delimiter or
        # quotechar value". Se corrige usando el quotechar estándar '"'.
...
try:
   ...
   # -------------------------------------------------------------------------
    ...
    # -------------------------------------------------------------------------
    # REGIÓN EN OPERACIONES (MODIFICACIÓN DEL EQUIPO 8 - Punto 5)
    # -------------------------------------------------------------------------
    print("REGIÓN EN OPERACIONES")
    valorizarIdRegion(connection, cursor)
